...
app.get('/api/tweets', async (req, res) => {
  try {
    ...
    if (!hasCompletedInitialSearch) {
      ...
      // Get all tweets from storage, sorted by engagement
      const allTweets = Array.from(tokenTweets.values())
        .sort((a, b) => {
          const engagementA = a.tweet.metrics.likes + a.tweet.metrics.retweets;
          const engagementB = b.tweet.metrics.likes + b.tweet.metrics.retweets;
